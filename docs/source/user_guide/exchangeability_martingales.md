...
`PluginKDEMartingale` is intentionally deferred to a later release.

## Why P-values (Not Raw Scores)

These martingales are conformal/exchangeability tests. Their validity relies on:

- under exchangeability, conformal p-values are approximately i.i.d. Uniform(0, 1)
...
Uses `r_n = epsilon * p_n^(epsilon - 1)`, `epsilon in (0, 1]`.
...
`MartingaleState.triggered_alarms` returns which alarms are currently exceeded.
