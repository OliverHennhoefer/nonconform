...
Next, let $\tilde{R}_i$ indicate the number of rejections obtained by applying BH at level $\alpha$, for some fixed $\alpha \in (0,1)$, to the approximate p-values $(\tilde{u}_i(X_{n+1}), \dots, \tilde{u}_i(X_{i-1}), 0, \tilde{u}_i(X_{i+1}), \tilde{u}_i(X_{i+2}), \dots, \tilde{u}_i(X_{n+m}))$.
...
Then, as in Section~\ref{sec:fdr-3steps}, let $\tilde{R}_i$ indicate the number of rejections obtained by applying BH at level $\alpha$, for some fixed $\alpha \in (0,1)$, to the approximate conformal p-values $(\tilde{u}_i(X_{n+1}), \dots, \tilde{u}_i(X_{i-1}), 0, \tilde{u}_i(X_{i+1}), \tilde{u}_i(X_{i+2}), \dots, \tilde{u}_i(X_{n+m}))$.
