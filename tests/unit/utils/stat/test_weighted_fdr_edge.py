...
class TestNoDiscoveries:
    def test_all_high_p_values(self, conformal_result):
        ...
        discoveries = false_discovery_control(result.p_values, method="bh") <= 0.05
...
class TestAllDiscoveries:
    def test_all_low_p_values(self, conformal_result):
        ...
        discoveries = false_discovery_control(result.p_values, method="bh") <= 0.5
        assert isinstance(discoveries, np.ndarray)
...
class TestSingleTestPoint:
    ...
    def test_single_test_point_high_p_value(self, conformal_result):
        ...
        discoveries = false_discovery_control(result.p_values, method="bh") <= 0.01
        assert len(discoveries) == 1
...
class TestInvalidPValues:
    ...
    def test_invalid_p_values_bh(self, conformal_result, bad_value):
        ...
        with pytest.raises(ValueError):
            _ = false_discovery_control(result.p_values, method="bh")

    def test_invalid_p_values_shape_bh_is_accepted(self):
        p_values_2d = np.array([[0.1], [0.2]])
        adjusted = false_discovery_control(p_values_2d, method="bh")
        discoveries = adjusted <= 0.1
        assert discoveries.shape == (2, 1)
